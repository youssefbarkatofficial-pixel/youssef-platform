...
(function(){
  ...
  function getSmartResponse(text) {
    ...
    if (/عامل ايه|ازيك|ايه اخبارك|وضعك ايه|صباح الخير|مساء الخير/.test(normalized)) {
      return { text: 'الحمد لله يا بطلطŒ ربنا معاك. قلظ‘ي إيه المطلوب وأنا أظبطلك الرد.' };
    }
    if (/انت مين|مين انت|انت مين/.test(normalized)) {
      return { text: 'أنا البوصلة بتاعتك في المنصةطŒ صايع الردود ودايمظ‹ا معاك في المذاكرة والدعم.' };
    }
    if (/مش قادر.*ذاكر|مش قادر.*الذاكر|معرفش|ماعرفش|مش عرف|مش فاهم|مش عارف\b/.test(normalized)) {
      return { text: 'ولا يهمكطŒ كلنا بنمر بده. قسظ‘م المذاكرة على أجزاء صغيرة وأبدأ بحاجة بسيطةطŒ وأنا أساندك خطوة بخطوة.' };
    }
    if (/خايف.*الامتحان|خايف من الامتحان|قلقان.*الامتحان/.test(normalized)) {
      return { text: 'الخوف ده طبيعيطŒ بس لما تكون منظم وبتراجع صح هتلاقي نفسك مرتاح. قولظ‘ي انت محتاج تذاكر في إيه بالظبط.' };
    }
    ...
    if (/انت ذكي|كويس|ممتاز/.test(normalized)) {
      return { text: 'بحاول أكون كويس دلوقتيطŒ بس اللي فعلاظ‹ ذكي هو اللي بيتعب ويبذل مجهود.' };
    }
  ...
  function ruleAnswerFor(text){
    ...
    if (isVeryUnclearMessage(text)) {
      return { text: 'معلش يا جميل â‌¤ï¸ڈ\nممكن تكتب سؤالك بشكل أوضح شوية عشان أعرف أساعدك صحطں' };
    }
    ...
    if (/حل السؤال ده|حل السؤال|عايز حل/.test(normalized)) {
      ...
      return { text: 'عشان أساعدك صحطŒ قولظ‘ي السؤال اللي واقف معاك عشان أشرحلك الفكرة مش أحلهلك.' };
    }
