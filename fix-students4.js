...
const endFuncStr = "function renderStudentsTable(filter = '') {";
const endFunc = html.indexOf(endFuncStr);

if (endFunc > -1) {
    const onSnapStart = html.lastIndexOf('if (window.firebaseDb) {', endFunc);
    if (onSnapStart > -1) {
        ...
        html = html.substring(0, onSnapStart) + replacement + '\n\n            ' + html.substring(endFunc);
        fs.writeFileSync('admin-students.html', html);
        console.log('Fixed admin-students.html successfully!');
    } else {
        console.log('onSnapStart not found');
    }
} else {
    console.log('endFunc not found');
}
